...
const DEFAULT_ROADMAP = [
  { phase: 1, title: "Foundations", duration: "Months 1–3", color: "#7eb8f7", sections: [
    ...
    { id: "cli-git", title: "Command Line & Git", weeks: "Week 1 (do this first!)", why: "Every single DS job requires Git. Showing up to an interview without Git knowledge signals that you've never worked on a real project.", warning: "Most people learn Git by memorizing commands. Learn the mental model first: what is a commit, what is a branch, what is a remote.", goldAdvice: "Your GitHub profile is your DS resume. Green squares matter. Push something every day, even if it's small.", resources: [{ name: "The Missing Semester of CS Education", type: "Free MIT course", url: "https://missing.csail.mit.edu" }, { name: "Learn Git Branching", type: "Free interactive", url: "https://learngitbranching.js.org" }], tasks: ["Navigate folders: cd, ls, mkdir, rm, cp, mv", "Run Python scripts from terminal", "Virtual environments: python -m venv, activate, pip install", "Git init, add, commit, push, pull", "Branching: create, switch, merge branches", "Write meaningful commit messages (not 'update' or 'fix')", "Create a professional GitHub profile with a README", "Push every project to GitHub with a proper README"] },
    { id: "jupyter", title: "Jupyter & Dev Environment", weeks: "Week 1–2 (parallel)", why: "Bad notebook habits will slow you down for years. Learn to use your tools properly from the start.", warning: "Notebooks with cells run out of order are not reproducible. Always restart and run all before sharing.", goldAdvice: "Learn keyboard shortcuts for Jupyter — they save hours. Also set up VS Code for Python early — it's what professionals use.", resources: [{ name: "VS Code Python setup", type: "Docs", url: "https://code.visualstudio.com/docs/python/python-tutorial" }], tasks: ["Set up VS Code with Python extension", "Learn Jupyter keyboard shortcuts (A, B, DD, Shift+Enter, M, Y)", "Always restart kernel and run all before sharing", "Use markdown cells to document your thinking", "Understand the difference between script (.py) and notebook (.ipynb)", "Know when to use a notebook vs a script"] },
  ]},
  { phase: 2, title: "Core ML", duration: "Months 3–7", color: "#6dd6a0", sections: [
    ...
    { id: "timeseries", title: "Time Series", weeks: "Week 11–12", why: "Time series comes up in 40% of DS jobs — finance, retail, operations, forecasting. Almost no bootcamp teaches it properly.", warning: "Time series has a leakage trap that catches everyone: you cannot use future data to predict the past. Always use time-based train/test splits.", goldAdvice: "Master the decomposition first: trend + seasonality + residual. If you can explain these three components with a real example, you're ahead of most candidates.", resources: [{ name: "Forecasting: Principles and Practice", type: "Free online book", url: "https://otexts.com/fpp3/" }, { name: "statsmodels docs", type: "Docs", url: "https://www.statsmodels.org" }], tasks: ["Time series decomposition: trend, seasonality, residual", "Stationarity — ADF test, differencing", "ARIMA and SARIMA — intuition and fitting", "Prophet by Meta — fast and practical", "Time-based train/test split — never random split", "Evaluation metrics: MAE, RMSE, MAPE", "Build a real forecasting project: sales, weather, or stock prices"] },
    { id: "eda-tools", title: "EDA & Model Explainability", weeks: "Week 9–10 (parallel)", why: "Companies don't just want predictions — they want to understand WHY the model predicts what it does. SHAP is now expected in interviews.", warning: "Don't just run ydata-profiling and call it EDA. Use it to find questions, then answer them manually with code.", goldAdvice: "SHAP waterfall plots in a presentation are a conversation-starter in every interview. Learn to read and explain them fluently.", resources: [{ name: "SHAP docs", type: "Docs", url: "https://shap.readthedocs.io" }, { name: "ydata-profiling", type: "Free tool", url: "https://github.com/ydataai/ydata-profiling" }], tasks: ["ydata-profiling: generate automated EDA report", "sweetviz: compare train vs test distributions", "SHAP values: global feature importance", "SHAP waterfall plot: explain single prediction", "LIME: explain any black-box model locally", "Present SHAP results as a business insight, not just a chart"] },
    { id: "api-data", title: "APIs & Real Data Sources", weeks: "Week 10 (parallel)", why: "Real data comes from APIs, not Kaggle CSVs. Every DS job involves pulling data from APIs at some point.", warning: "Never hardcode API keys in your code. Use environment variables. This is a professionalism signal.", goldAdvice: "Build one project using a real public API — weather, finance, Twitter, Reddit. It immediately looks more impressive than another Titanic project.", resources: [{ name: "requests library docs", type: "Docs", url: "https://requests.readthedocs.io" }, { name: "Public APIs list", type: "Free", url: "https://github.com/public-apis/public-apis" }], tasks: ["HTTP basics: GET, POST, headers, status codes", "requests library: fetch data from any API", "Handle pagination and rate limits", "Parse JSON responses into Pandas DataFrames", "Use environment variables for API keys (.env file)", "Build a small project using a real public API"] },
  ]},
  { phase: 3, title: "Modern Skills", duration: "Months 7–12", color: "#f7c96e", sections: [
    ...
    { id: "streamlit", title: "Streamlit — Deploy ML Apps", weeks: "Week 8 (parallel)", why: "'Here's my live demo' is the most powerful thing you can say in an interview. Streamlit lets you build and deploy a web app for any model in hours.", warning: "Don't over-engineer your Streamlit app. A simple, clean app that works is 10x better than a complex one that crashes.", goldAdvice: "Every project you build from Phase 2 onwards should have a Streamlit demo. Deploy it to Streamlit Cloud for free — keep it live.", resources: [{ name: "Streamlit docs", type: "Docs", url: "https://docs.streamlit.io" }, { name: "Streamlit Cloud", type: "Free hosting", url: "https://streamlit.io/cloud" }], tasks: ["Build your first Streamlit app in under 1 hour", "Add file upload, sliders, dropdowns, and charts", "Deploy to Streamlit Cloud — get a live URL", "Wrap your churn model in a Streamlit app", "Add SHAP explanations to your Streamlit app", "Connect to a real data source (CSV upload or API)"] },
    { id: "cloud", title: "Cloud Basics (AWS/GCP)", weeks: "Week 11–12", why: "Every DS job mentions cloud. You don't need to be a cloud engineer but you need to be comfortable with the basics.", warning: "Don't try to learn all cloud services at once. Focus on storage (S3), compute (EC2 or Colab), and deployment (one service end to end).", goldAdvice: "Get the AWS Free Tier account and actually deploy something — even a simple model API. Having a real AWS experience beats any certification.", resources: [{ name: "AWS Free Tier", type: "Free account", url: "https://aws.amazon.com/free/" }, { name: "Google Colab Pro", type: "Affordable GPU", url: "https://colab.research.google.com" }, { name: "A Cloud Guru", type: "Course", url: "https://acloudguru.com" }], tasks: ["Set up AWS Free Tier account", "S3: upload, download, manage files programmatically with boto3", "EC2: launch a basic instance, SSH into it", "Deploy a FastAPI model to EC2 — get a live URL", "Understand IAM roles and basic security", "Google Colab: use free GPU for training", "Understand the difference: IaaS vs PaaS vs SaaS"] },
  ...
  { phase: 4, title: "Portfolio & Jobs", duration: "Months 12–18", color: "#c792ea", sections: [
    ...
    { id: "kaggle", title: "Kaggle Competitions", weeks: "Month 10–14 (parallel)", why: "A top 30% finish on Kaggle shows you can compete against thousands of real data scientists. It's objective proof of skill.", warning: "Don't join a competition just to submit once. Spend time reading other people's notebooks — you'll learn more from them than from courses.", goldAdvice: "Your first competition will be humbling. That's fine. Focus on learning from the leaderboard, not your rank.", resources: [{ name: "Kaggle competitions", type: "Free", url: "https://www.kaggle.com/competitions" }, { name: "Kaggle Learn", type: "Free mini-courses", url: "https://www.kaggle.com/learn" }], tasks: ["Complete at least one Getting Started competition (Titanic, House Prices)", "Enter one real competition — any tabular data one", "Read top 3 solution writeups after competition ends", "Aim for top 30% on at least one competition", "Write a Kaggle notebook that gets upvotes", "Connect with other Kagglers in your domain"] },
    { id: "linkedin-profile", title: "LinkedIn Optimization", weeks: "Month 12–13", why: "Recruiters search LinkedIn every day. A well-optimized profile gets you inbound messages — you don't have to apply cold.", warning: "Most DS candidates have terrible LinkedIn profiles: no summary, no projects, generic headlines. Standing out is easier than you think.", goldAdvice: "Your headline should not say 'Data Science Student'. Say 'Data Scientist | ML Engineer | NLP & MLOps | Open to opportunities'. Keywords matter for search.", resources: [{ name: "LinkedIn for Job Seekers", type: "Free guide", url: "https://linkedin.com" }], tasks: ["Professional headshot — this matters more than people admit", "Headline: include your specialization and 'Open to opportunities'", "Summary: 3 paragraphs — who you are, what you build, what you're looking for", "Add all projects with live demo links", "Get 3+ recommendations from professors, mentors, or peers", "Turn on 'Open to Work' (visible to recruiters only)", "Post at least 4 times before you start applying", "Connect with 50+ DS professionals in your target companies"] },
    { id: "salary-neg", title: "Salary Negotiation", weeks: "Month 16–18", why: "Most people leave 10-20% of their salary on the table by not negotiating. This compounds over your entire career.", warning: "Never give the first number. Always say 'I'd like to understand the full compensation package before discussing numbers'.", goldAdvice: "Research salaries on Glassdoor, Levels.fyi, and LinkedIn Salary before any interview. Know your number before they ask.", resources: [{ name: "Levels.fyi", type: "Salary data", url: "https://www.levels.fyi" }, { name: "Glassdoor", type: "Salary & reviews", url: "https://www.glassdoor.com" }, { name: "Ten Rules for Negotiating — Haseeb Qureshi", type: "Free article (must read)", url: "https://haseebq.com/my-ten-rules-for-negotiating-a-job-offer/" }], tasks: ["Research market salaries for DS roles in your target location", "Learn the BATNA concept — know your walk-away number", "Practice saying 'I need time to consider' out loud", "Negotiate total comp: base + bonus + equity + benefits", "Never accept on the spot — always ask for 48 hours", "Read Haseeb Qureshi's negotiation guide — it's the best one", "Practice the negotiation conversation with a friend"] },
  ]},
];
